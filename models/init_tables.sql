...
CREATE TABLE teachers(id SERIAL PRIMARY KEY, lastname VARCHAR(255) not null, department INT REFERENCES departments(id) ON DELETE CASCADE);
CREATE TABLE courses(id VARCHAR(255) PRIMARY KEY not null CHECK(id <> ''), department INT REFERENCES departments(id) ON DELETE CASCADE, coursename VARCHAR(255), shortname VARCHAR(255), _9 int DEFAULT 0, _10 int DEFAULT 0, _11 int DEFAULT 0, _12 int DEFAULT 0, semester INT DEFAULT 3, sections INT DEFAULT 1, color VARCHAR(255) DEFAULT 'FFFFFF');
CREATE TABLE blocks(id SERIAL PRIMARY KEY, teacher_id INT REFERENCES teachers(id) ON DELETE CASCADE, course_id VARCHAR(255) REFERENCES courses(id) ON DELETE CASCADE, room VARCHAR(255), time INT, seats INT, UNIQUE (time, teacher_id));
