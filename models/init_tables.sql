...
CREATE TABLE courses(id VARCHAR(255) PRIMARY KEY not null CHECK(id <> ''), department INT REFERENCES departments(id), coursename VARCHAR(255), shortname VARCHAR(255), _9 int DEFAULT 0, _10 int DEFAULT 0, _11 int DEFAULT 0, _12 int DEFAULT 0);
CREATE TABLE blocks(id SERIAL PRIMARY KEY, teacher_id INT REFERENCES teachers(id), course_id VARCHAR(255) REFERENCES courses(id), room INT, time INT);
